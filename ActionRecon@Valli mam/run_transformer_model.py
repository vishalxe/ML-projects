#!/usr/bin/env python3
"""
Transformer Model for Human Activity Recognition - Standalone Execution Script
This script runs the Transformer encoder model and generates comprehensive results for journal publication.
"""
...
class TransformerModelRunner:
    ...
    def create_transformer_har_model(self, input_shape, num_classes, d_model=64, 
                                   num_heads=8, num_layers=2, dff=128, dropout_rate=0.1):
        ...
        # Add positional encoding through a Keras layer to avoid using KerasTensor in raw TF ops
        x = Lambda(lambda t: t + pos_encoding_tensor[:, :tf.shape(t)[1], :])(x)
        
        # Stack of Transformer encoder blocks
        for _ in range(num_layers):
            transformer_block = self.create_transformer_encoder_block(
                d_model=d_model,
                num_heads=num_heads,
                dff=dff,
    ...
    def train_model(self, X_train, y_train, X_test, y_test):
        ...
        self.model = self.create_transformer_har_model(input_shape, num_classes)
        ...
        callbacks = [
            ...
            ModelCheckpoint('best_transformer_model.keras', monitor='val_accuracy', save_best_only=True, verbose=1)
        ]
    ...
    def generate_plots(self, label_encoder):
        ...
        plt.title('Transformer - Training Accuracy', fontweight='bold')
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy')
        plt.legend()
        ...
        plt.title('Transformer - Training Loss', fontweight='bold')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.title('Transformer - Confusion Matrix', fontweight='bold')
        ...
        plt.title('Transformer - ROC Curves')
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(True, alpha=0.3)
        
        # Model Summary
        plt.subplot(2, 3, 5)
        plt.text(0.1, 0.8, 'Model: Transformer Encoder', fontsize=12, fontweight='bold')
        ...
        plt.ylim(0, 1)
        plt.axis('off')
        plt.title('Transformer - Performance Summary', fontweight='bold')
    ...
    def save_results(self):
        ...
        # Save model
        self.model.save('transformer_model_final.keras')
...
def main():
    ...
    print("=== Transformer Model for Human Activity Recognition ===")
    
    # Initialize model runner
    csv_path = r"c:\Users\Vishal\Downloads\HAR_synthetic_full\HAR_synthetic_full.csv"
    runner = TransformerModelRunner(csv_path)
